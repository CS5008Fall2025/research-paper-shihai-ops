...
```
To calculate the time complexity of insertion for an AVL tree, we need to first check the minimum Number(n) of nodes needed to build a balanced tree(T) with height(h).  A balanced tree with minimum number of node $T_n$ consists of $T_{n-1}$ and $T_{n-2}$. 
## Empirical Analysis
...
class AVLTree:
    ...
    def _insert(self, node, key):  # insert key just like bst, first insert the root, if the root 
        ...
        # Case 3 - Left Right                        # if the balance > 1 (left is higher than right), and
        if balance > 1 and key > node.left.key:      # new node is bigger than the left child,
